...
def validate_email(email):
    ...
    while index < len(email):
        ...
        index += 1
    if counter > 3:
        raise ValueError("Extension maximum length id 3")
    return None
...
def test_username_can_contain_letters_and_digits():
    assert validate_email("a1234@nomewebsite.ext") is None


def test_username_can_contain_letters_and_digits_and_dashes():
    assert validate_email("aa-a@nomewebsite.ext") is None


def test_username_can_contain_letters_and_digits_and_dashes_and_underscores():
    assert validate_email("a_a1a@nomewebsite.ext") is None
...
def test_username_when_doesnt_start_with_letters_raise_exception():
    with pytest.raises(ValueError):
        validate_email("1@nomewebsite.txt")


def test_username_is_invalid_raise_exception():
    with pytest.raises(ValueError):
        validate_email("a%@nomewebsite.txt")
